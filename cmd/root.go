...
func newRootCmd() *cobra.Command {
	root := &cobra.Command{
		...
		Long:         "Manage CloudFormation install and upgrade workflows for a Nuon install. Both commands require --inputs (a JSON object map of CloudFormation parameters). Provide changed secrets via --secrets; on stack updates, omitted or empty template secret values keep existing stack values.",
